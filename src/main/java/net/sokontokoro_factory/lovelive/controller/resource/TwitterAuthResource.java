...
public class TwitterAuthResource {
  ...
  @Path("login")
  @GET
  public Response login(@QueryParam("redirect") @DefaultValue("/") String redirectPath) {
    ...
    final OAuth10aService service =
        new ServiceBuilder(TWITTER_APIKEY)
    ...
    // twitter認証画面へリダイレクト
    URI redirect =
        UriBuilder.fromUri("https://api.twitter.com")
            .path("oauth")
            .path("authorize")
            .queryParam("oauth_token", requestToken.getToken())
            .build();
  ...
  public Response callback(
    ...
    if (requestToken == null) {
      return Response.status(Status.UNAUTHORIZED)
          .entity("Unauthorized.Please try to login again, sorry.")
          .build();
    }

    URI redirect =
        UriBuilder.fromUri(GAME_CLIENT_ORIGIN)
  ...
  @Path("logout")
  @GET
  public Response logout(@QueryParam("redirect") @DefaultValue("/") String redirectPath) {

    URI redirect = UriBuilder.fromUri(GAME_CLIENT_ORIGIN).path(redirectPath).build();
