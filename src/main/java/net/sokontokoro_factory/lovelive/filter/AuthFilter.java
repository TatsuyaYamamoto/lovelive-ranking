...
public class AuthFilter implements ContainerRequestFilter {
  ...
  @Inject private LoginSession loginSession;

  /** アノテーション定義用内部クラス NGの場合にログインページヘredirctionするかをredirect(引数)で指定する */
  ...
  /**
   * "@LoginRequired"がアノテートされたリソース(リソースクラスのメソッド)へのリクエストに対しての検証フィルター。 ログイン済みでない場合、401を返却する
   *
  ...
  public void filter(ContainerRequestContext requestContext) throws IOException {
    ...
    boolean isAnnotated =
        resourceInfo.getResourceMethod().getAnnotation(LoginRequired.class) != null;
    if (!isAnnotated) {
      return;
    }
